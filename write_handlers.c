...
{ /* char is stored at left and paddind at buffer's right */
	...
	/* UNUSED macro is used to prevent compiler warnings for unused parameters. */
	UNUSED(precision);
	UNUSED(size);
	...
	if (width > 1)
	{
		...
		/* Add padding to the buffer for the specified width. */
		for (i = 0; i < width - 1; i++)
...
/**
 * write_num - Writes a number into a buffer with specified formatting.
 *
...
{
	int i, padd_start = 1;
	/* Check if precision is 0, and the number is zero, for special formatting. */
...
/**
 * write_number - Prints a formatted number (possibly negative).
 *
